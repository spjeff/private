...
.NOTES
    ...
    Author   :  Jeff Jones <jeff.jones@butterflytech.net> Galen Keene <galen.keene@butterflytech.net>
	Version  :  1.4
    Modified :  2020-07-19
...
# Config.  Replace when running "BTF-Calendar-Setup.ps1"
...
# $sqlInst = "BFTECH-GKNUC-01"
# $sqlDb   = "bftech-usc-cal"
# $sqlUser = "uscal"
# $sqlPass = "UscCal"
...
function cleanSQL($text) {
    # Escape single quote.  Hande NULL input gracefully.
    if ($text) {
...
function readCalendar($token) {
    # SQL list of users
    $tsql = "SELECT * FROM [ExemptEmployees]"
    $users = Invoke-Sqlcmd -Database $sqlDb -ServerInstance $sqlInst -Username $sqlUSer -Password $sqlPass -Query $tsql
    ...
    foreach ($u in $users) {
        ...
        do {
            foreach ($r in $events.value) {
                ...
                # Duration (Hours)
                $hours = 0
                $span = [datetime]$r.end.dateTime - [datetime]$r.start.dateTime
                $hours = $span.TotalHours;

                # Attendees
                $r.attendees | % { 
                    # Check if current user in ExemptEmployees SQL table
                    $aupn = $_.emailAddress.address
                    $found = $null
                    $found = $global:ExemptEmployees | ? { $_.address -eq $aupn }
                    if ($found) {
                        $totalExemptHours += $hours
                    }
                    else {
                        $totalOtherHours += $hours
                    }
                }

                # Organizer
                $orgfound = $null
                $orgfound = $r.attendees |? {$_.emailAddress.address -eq $r.organizer.emailAddress.address}
                if (!$orgfound) {
                    $found = $null
                    $found = $global:ExemptEmployees | ? { $_.address -eq $r.organizer.emailAddress.address }
                    if ($found) {
                        $totalExemptHours += $hours
                    }
                    else {
                ...
                # SQL
                $id = $r.id
                $subject = cleanSQL $r.subject
                $bodyPreview = cleanSQL $r.bodyPreview
                $attend = cleanSQL $attend
                ...
                $tsql = "INSERT INTO [Events] VALUES ('$id','$subject','$bodyPreview','$start','$end','$attend',$attendeesCount,'$organizername','$organizeraddress','$upn',$totalExemptHours,$totalOtherHours)"
                Write-Host $tsql
...
function readExempt() {
    $tsql = "SELECT * FROM [dbo].[ExemptEmployees] WHERE [isExempt]=1"
