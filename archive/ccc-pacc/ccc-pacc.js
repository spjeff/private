...
var urlCancel = 'https://cccedu.sharepoint.com/sites/PAC/automated/dev/SitePages/Step%201%20-%20PA%20Cancel%20Proposal.aspx';
var urlSaveClose = 'https://cccedu.sharepoint.com/sites/PAC/automated/dev/SitePages/Step%201%20-%20PA%20Cancel%20Proposal.aspx';
var urlSaveSubmit = 'https://cccedu.sharepoint.com/sites/PAC/automated/dev/SitePages/Step%201%20-%20PA%20Successfully%20Initiated%20Proposal.aspx';
var urlSave = '';
...
function paccCtl($scope) {
  ...
  // Is current user PA?
  vm.isPA = function () {
    return vm.currentUser.Email == vm.d.step1.advocateEmail;
  }
  ...
  vm.isRecommender = function () {
    ...
    if (vm.recommenders) {
      angular.forEach(vm.recommenders, function (row) {
        if (row.Recommenders[0].EMail == vm.currentUser.Email) {
          match = true;
          return;
        }
      })
  ...
  vm.notValid = function () {
    ...
    if (vm.d.status == 1) {
      if (!vm.d.step1.discipline.Title ||
  ...
  vm.currentCF = function () {
    ...
    // CF
    if (vm.currentUser) {
      if (vm.currentUser.Email == vm.d.curriculumFacilitatorEMail) {
        return true;
      }
    }
    ...
    // DCF
    if (vm.currentUser) {
      if (vm.currentUser.Email == vm.d.dcurriculumFacilitatorEMail) {
        return true;
      }
    }
  ...
  vm.step2vote = function (role, vote) {
    ...
    // Move to Step 3
    if (vm.d.step2.CF && vm.d.step2.DES && vm.d.step2.DEAN) {
      // Total votes
      var total = 0;
      if (vm.d.step2.CF) total++;
      if (vm.d.step2.DES) total++;
      if (vm.d.step2.DEAN) total++;
      ...
      if (vm.d.step2.DEAN.indexOf(accept) > -1) {
        approveTotal++;
      }
