            ...
            if (events.Count > 0)
            {
                foreach (EventClass e in events)
                {
                    ...
                    if (count == 0 && e.Subject != "OOO" && !e.Subject.EndsWith("-OOO") && !e.Subject.EndsWith(" OOO"))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Subject", e.Subject.Truncate(1000));
                        cmd.Parameters.AddWithValue("@Location", e.Location.Truncate(1000));
                        cmd.Parameters.AddWithValue("@Body", e.Body.Truncate(8000));
                        cmd.Parameters.AddWithValue("@IsRecurring", e.IsRecurring);
                        cmd.Parameters.AddWithValue("@Organizer", e.Organizer.Truncate(1000));
                        cmd.Parameters.AddWithValue("@OptionalAttendees", e.OptionalAttendees.Truncate(8000));
                        cmd.Parameters.AddWithValue("@RequiredAttendees", e.RequiredAttendees.Truncate(8000));
